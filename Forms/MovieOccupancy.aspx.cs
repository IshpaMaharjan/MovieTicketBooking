            ...
            using (OracleConnection conn = new OracleConnection(connStr))
            {
                ...
                string query = @"
                    SELECT 
                        ...
                        CAST(SUM(
                            CASE 
                                WHEN UPPER(T.TicketStatus) = 'BOOKED' THEN 1 
                                ELSE 0 
                            END
                        ) AS INTEGER) AS BookedTickets,
                        ...
                        AS INTEGER) AS AvailableTickets
                    ...
                    WHERE M.MovieTitle = :mname

                    GROUP BY M.MovieTitle, S.ShowDate";

                OracleCommand cmd = new OracleCommand(query, conn);

                // SAME PARAMETER (no change)
                cmd.Parameters.Add("mname", txtMovieId.Text);
